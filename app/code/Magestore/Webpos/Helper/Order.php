...
class Order extends \Magestore\Webpos\Helper\Data
{
    ...
    public function __construct(
        ...
        parent::__construct($context,$storeManager,$webPosSession,$checkoutSession, $customerSession,$localeDate,$dateTime,$storeRepository);
    }
    ...
    public function createShipmentAndInvoice($orderId, $order, $create_invoice, $create_shipment, $items_to_ship = [], $tracks = []) {
        ...
        if ($order->getId()) {
            ...
            if ($create_shipment) {
                try {
                    $shipment = (count($items_to_ship) > 0) ? $this->_prepareShipment($order, $items_to_ship) : $this->_prepareShipment($order);
                    if ($shipment) {
