...
class CreditdiscountBeforetax extends \Magento\Quote\Model\Quote\Address\Total\AbstractTotal
{
    /**
    ...
    public function __construct(
        ...
        $this->setCode('creditdiscountbeforetax');
    ...
    public function collect(
        ...
        if($this->_creditHelper->getSpendConfig('tax',$quote->getStoreId()) == '1'){
            return $this;
        }
    ...
    {
        $result = null;
        if($this->_creditHelper->getSpendConfig('tax',$quote->getStoreId()) == '1' || !$this->_accountHelper->customerGroupCheck()){
            return $result;
        }
            ...
            if ($item->getHasChildren() && $item->isChildrenCalculated()) {
                foreach ($item->getChildren() as $child) {
                    if (!$child->isDeleted() && $child->getProduct()->getTypeId() != 'customercredit') {
                        ...
                        $child->setCustomercreditDiscount($itemDiscount);

                        $child->setBaseDiscountAmount($child->getBaseDiscountAmount() + $itemBaseDiscount);
                        $child->setDiscountAmount($child->getDiscountAmount() + $itemDiscount);
                ...
                if (!$item->isDeleted() && $item->getProduct()->getTypeId() != 'customercredit') {
                    ...
                    $item->setCustomercreditDiscount($itemDiscount);
