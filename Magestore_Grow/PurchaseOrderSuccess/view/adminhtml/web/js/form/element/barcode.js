...
define([
    ...
    return Element.extend({
        defaults: {
            ...
            valueUpdate: false,
            focused: false,
            template: 'Magestore_PurchaseOrderSuccess/form/element/barcode',
            barcodeJson: {},
            sourceElment: '',
            destinationElement: '',
            selectionsProvider: '',
        ...
        onUpdate: function () {
            ...
            this.validate();
            if(this.value()!=''){
                if(typeof this.barcodeJson[this.value()] != 'undefined'){
                    ...
                    var des = registry.get(this.destinationElement),
                        ...
                        index = gridData.map(function(el) {return el[indexField];}).indexOf(indexValue);
                    if(index==-1){
                    ...
                    //         des = registry.get(this.destinationElement),
                    //         map = des.map,
                    //         indexField = des.identificationProperty;
                    // }
                    // if(selected && selected.indexOf(this.barcodeJson[this.value()][map[indexField]]) == -1) {
                    //     selected.push(this.barcodeJson[this.value()][map[indexField]]);
                    //     var source = registry.get(this.sourceElement);
                    //     source.save().done(function(){
                    //         this.updateInputValue().done(function(){this.value('')}.bind(this));
                    //     }.bind(this));
                    // }else{
                    //     this.updateInputValue().done(function(){this.value('')}.bind(this));
                    // }
