...
class DataProvider extends AbstractProvider
{
    ...
    public function __construct(
        ...
        $this->stockState = $stockStateInterface;
    ...
    public function getData()
    {
        ...
        if($this->type_provider == 'form') {
            ...
            if($id = $item->getProductId()) {
                ...
                $product->load($id);
                $qty = $this->stockState->getStockQty($id, $product->getStore()->getWebsiteId());
                $stockAvailability = $this->stockRegistry->getStockStatus($id)->getStockStatus();
                $origImageHelper = $this->imageHelper->init($product, 'product_listing_thumbnail_preview');

                $isInventoryEnabled = $this->helper->isModuleOutputEnabled('Magestore_InventorySuccess');
                ...
                $this->loadedData[$item->getId()]['product_qty'] = ($isInventoryEnabled)?$this->generateQtyHtml($this->helper->getWarehousesStock($id)):$qty;
     ...
     * @return string
     */
    public function generateQtyHtml($stock_data){
        $html = "<table class='warehouse_qty_table'>";
        $html .="<thead><th class='text-center'>".__('Warehouse')."</th><th class='text-center'>".__('Available Qty')."</th><th class='text-center'>".__('Qty to Ship')."</th><th class='text-center'>".__('Qty In Warehouse')."</th><th class='text-center'>".__('Shelf Location')."</th></thead>";
        $html .="<tbody>";
