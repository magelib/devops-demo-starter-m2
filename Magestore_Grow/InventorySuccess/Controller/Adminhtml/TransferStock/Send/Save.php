...
class Save extends \Magestore\InventorySuccess\Controller\Adminhtml\TransferStock\AbstractTransfer
{
    ...
    public function execute()
    {
        ...
        if (!empty($data)) {
            //$model->setData($data);
            $locator->setSesionByKey("current_send_stock", $data);
        }
        ...
        elseif ($data) {
                ...
                switch ($action){
                    ...
                    case "start_send":
                        if(!isset($data['links']['send_products'])){
                            ...
                            if($save_result){
                                $this->messageManager->addSuccessMessage(__('sent '. count($send_products) . ' product(s) to the destination warehouse!'));
                    ...
                    case "direct_transfer":
                        if(!isset($data['links']['send_products'])){
                            ...
                            if($save_result){
                                $this->messageManager->addSuccessMessage(__('Sent ' . count($send_products) . ' product(s) directly to the destination warehouse!'));
            ...
            try {
                ...
                $locator->refreshSessionByKey("current_send_stock");
    ...
    public function prepareProductData($receiving_products , $type){
        if($type == TransferActivityModel::ACTIVITY_TYPE_RECEIVING) {
            foreach ($receiving_products as $i => &$value) {
                if ($value['qty_sent'] < ($value['qty_received'] + $value['qty'])) {
                    $value['qty'] = max($value['qty_sent'] - $value['qty_received'], 0);
                    if ($value['qty'] == 0) {
    ...
    public function saveTransferStockProduct($send_products, $updateStock, $directTransfer){
        if(!$this->validateStockDelivery($send_products)){
            $this->_locatorFactory->create()->setSesionByKey("send_products", $send_products);
            return false;
        }
