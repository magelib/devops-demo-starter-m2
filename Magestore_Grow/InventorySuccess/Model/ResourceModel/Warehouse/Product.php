...
class Product extends \Magestore\InventorySuccess\Model\ResourceModel\AbstractResource
{
    ...
    public function addStockDataToCollection($collection, $isFilterInStock)
    {
        ...
        $joinCondition = 'e.entity_id = warehouse_product.product_id';
        $joinCondition .= $this->getConnection()->quoteInto(
            ' AND warehouse_product.'. WarehouseProductInterface::WAREHOUSE_ID .' = ?',
            $warehouseId
        );
        $method = 'joinLeft';
        $collection->getSelect()->$method(
            ['warehouse_product' => $this->getMainTable()],
            $joinCondition,
            ['total_qty', 'qty_to_ship']
        );

        /* join to global stock */
        $joinCondition = $this->getConnection()->quoteInto(
        ...
        $compositeTypes = '(configurable, bundle, grouped)';

        $collection->getSelect()->columns([
            'is_salable' => "IF(e.type_id = 'configurable', stock_status_index.stock_status, IF((warehouse_product.total_qty - warehouse_product.qty_to_ship) > 0, 1, 0))",
            'stock_status' => "IF(e.type_id = 'configurable', stock_status_index.stock_status, IF((warehouse_product.total_qty - warehouse_product.qty_to_ship) > 0, 1, 0))",
        ]);

        /* filter stock status*/
        if ($isFilterInStock) {
            $collection->getSelect()->where(
                "IF(e.type_id = 'configurable', stock_status_index.stock_status, IF((warehouse_product.total_qty - warehouse_product.qty_to_ship) > 0, 1, 0)) = ?",
                1
            );
        }
